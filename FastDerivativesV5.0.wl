...
Print["Adding contribuions from x's"];
nm=Length[txttabred];
...
ivar=Flatten[Position[rcombos,Sort[{iatom,jatom}]],1][[1]]; (* gives the variable corresponding
...
qlines,diagnose,positions,kstart,kend},

diagnose=False;

{tout,natoms,npoly,nmono,nq}=CreateAdjointText[mnumber,pnumber,qnumber];
nvar=natoms (natoms-1)/2;
Print["{npoly,nmono,nq} = ",{npoly,nmono,nq}];
...
If[nq>0,
	tout=text1<>text1a<>tout<>text2;
	,
	tout=text1<>tout<>text2;
];
(* Fix dimensions *)
Print["   fix dimensions"];
z31047=3*natoms;
tout=StringReplace[tout,"nmono1047"->ToString[nmono]];
tout=StringReplace[tout,"npoly1047"->ToString[npoly]];
tout=StringReplace[tout,"natoms1047"->ToString[natoms]];
tout=StringReplace[tout,"xxxxx"->ToString[z31047]];
tout=StringReplace[tout,"nq1047"->ToString[nq]];
